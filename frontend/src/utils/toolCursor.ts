...
// at. The hotspot is expressed in the icon's own 16-unit box and scaled to this.
...
export function toolCursorCss(tool: AppTool): string {
  const { iconPath, hotspot } = toolById(tool)

  const svg =
    `<svg xmlns="http://www.w3.org/2000/svg" width="${CURSOR_SIZE}" height="${CURSOR_SIZE}" viewBox="0 0 16 16">` +
  ...
  const x = Math.round((hotspot[0] / 16) * CURSOR_SIZE)
  const y = Math.round((hotspot[1] / 16) * CURSOR_SIZE)
