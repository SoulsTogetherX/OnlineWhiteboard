//#region Imports
import type { ReactNode } from "react"

import type { CursorTool } from "@shared/types/socketProtocol"
//#endregion

//#region Types
// The toolbar's selectable tools. "eyedropper" is a frontend-only ACTION — it
// samples a colour rather than producing a draw instruction — but it is still
// something other people can see you holding, so the wire type that carries a
// cursor's tool covers it too. AppTool is that same set under the name the UI
// has always used for it.
export type AppTool = CursorTool

export interface ToolDescriptor {
  id: AppTool
  name: string
  // Single-key shortcut, shown in the tooltip and (Phase 5 keymap) bound while
  // the sidebar is open. Defined here so the picker and the keymap share one
  // source and can't disagree.
  shortcut: string
  // Whether this tool draws with the stroke-width control (pencil/eraser). The
  // contextual panel uses it to decide what to show.
  usesStroke: boolean
  // The glyph as raw path data in a 16x16 box, NOT as JSX.
  //
  // It is the path rather than an element because the same glyph has to become
  // two different things: a React icon (the picker, a remote cursor) and an SVG
  // data URI (the local CSS cursor). Keeping the geometry here means the tool
  // you hold and the tool everyone sees you holding can never drift apart.
  iconPath: string
  // Where the glyph's business end sits in that 16x16 box. This becomes the CSS
  // cursor hotspot, so a click lands where the tool's tip is drawn rather than
  // at the corner of its bounding box.
  hotspot: [number, number]
  icon: ReactNode
}
//#endregion

//#region Icon geometry
// Decorative — each descriptor's `name` carries the accessible label.
const PENCIL_PATH =
  "M15.825.12a.5.5 0 0 1 .132.584c-1.53 3.43-4.743 8.17-7.095 10.64a6.1 6.1 0 0 1-2.373 1.534c-.018.227-.06.538-.16.868-.201.659-.667 1.479-1.708 1.74a8.1 8.1 0 0 1-3.078.132 4 4 0 0 1-.562-.135 1.4 1.4 0 0 1-.466-.247.7.7 0 0 1-.204-.288.62.62 0 0 1 .004-.443c.095-.245.316-.38.461-.452.394-.197.625-.453.867-.826.095-.144.184-.297.287-.472l.117-.198c.151-.255.326-.54.546-.848.528-.739 1.201-.925 1.746-.896q.19.012.348.048c.062-.172.142-.38.238-.608.261-.619.658-1.419 1.187-2.069 2.176-2.67 6.18-6.206 9.117-8.104a.5.5 0 0 1 .596.04M4.705 11.912a1.2 1.2 0 0 0-.419-.1c-.246-.013-.573.05-.879.479-.197.275-.355.532-.5.777l-.105.177c-.106.181-.213.362-.32.528a3.4 3.4 0 0 1-.76.861c.69.112 1.736.111 2.657-.12.559-.139.843-.569.993-1.06a3 3 0 0 0 .126-.75zm1.44.026c.12-.04.277-.1.458-.183a5.1 5.1 0 0 0 1.535-1.1c1.9-1.996 4.412-5.57 6.052-8.631-2.59 1.927-5.566 4.66-7.302 6.792-.442.543-.795 1.243-1.042 1.826-.121.288-.214.54-.275.72v.001l.575.575zm-4.973 3.04.007-.005zm3.582-3.043.002.001h-.002z"

const ERASER_PATH =
  "M8.086 2.207a2 2 0 0 1 2.828 0l3.879 3.879a2 2 0 0 1 0 2.828l-5.5 5.5A2 2 0 0 1 7.879 15H5.12a2 2 0 0 1-1.414-.586l-2.5-2.5a2 2 0 0 1 0-2.828zm2.121.707a1 1 0 0 0-1.414 0L4.16 7.547l5.293 5.293 4.633-4.633a1 1 0 0 0 0-1.414zM8.746 13.547 3.453 8.254 1.914 9.793a1 1 0 0 0 0 1.414l2.5 2.5a1 1 0 0 0 .707.293H7.88a1 1 0 0 0 .707-.293z"

const BUCKET_PATH =
  "M6.192 2.78c-.458-.677-.927-1.248-1.35-1.643a3 3 0 0 0-.71-.515c-.217-.104-.56-.205-.882-.02-.367.213-.427.63-.43.896-.003.304.064.664.173 1.044.196.687.556 1.528 1.035 2.402L.752 8.22c-.277.277-.269.656-.218.918.055.283.187.593.36.903.348.627.92 1.361 1.626 2.068.707.707 1.441 1.278 2.068 1.626.31.173.62.305.903.36.262.05.64.059.918-.218l5.615-5.615c.118.257.092.512.05.939-.03.292-.068.665-.073 1.176v.123h.003a1 1 0 0 0 1.993 0H14v-.057a1 1 0 0 0-.004-.117c-.055-1.25-.7-2.738-1.86-3.494a4 4 0 0 0-.211-.434c-.349-.626-.92-1.36-1.627-2.067S8.857 3.052 8.23 2.704c-.31-.172-.62-.304-.903-.36-.262-.05-.64-.058-.918.219zM4.16 1.867c.381.356.844.922 1.311 1.632l-.704.705c-.382-.727-.66-1.402-.813-1.938a3.3 3.3 0 0 1-.131-.673q.137.09.337.274m.394 3.965c.54.852 1.107 1.567 1.607 2.033a.5.5 0 1 0 .682-.732c-.453-.422-1.017-1.136-1.564-2.027l1.088-1.088q.081.181.183.365c.349.627.92 1.361 1.627 2.068.706.707 1.44 1.278 2.068 1.626q.183.103.365.183l-4.861 4.862-.068-.01c-.137-.027-.342-.104-.608-.252-.524-.292-1.186-.8-1.846-1.46s-1.168-1.32-1.46-1.846c-.147-.265-.225-.47-.251-.607l-.01-.068zm2.87-1.935a2.4 2.4 0 0 1-.241-.561c.135.033.324.11.562.241.524.292 1.186.8 1.846 1.46.45.45.83.901 1.118 1.31a3.5 3.5 0 0 0-1.066.091 11 11 0 0 1-.76-.694c-.66-.66-1.167-1.322-1.458-1.847z"

const SPRAY_PATH =
  "M3 2a1 1 0 0 1 1-1h1a1 1 0 0 1 1 1v1.5a1 1 0 0 1 .5.866l.5.289a2 2 0 0 1 1 1.732V14a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1V6.387a2 2 0 0 1 1-1.732l.5-.289A1 1 0 0 1 5 3.5V2zm7.5 1a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0m2-1a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0m-2 3a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0m3 0a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0m-1.5 2.5a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0M14 5a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0m-1 3a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0"

const EYEDROPPER_PATH =
  "M13.354.646a1.207 1.207 0 0 0-1.708 0L8.5 3.793l-.646-.647a.5.5 0 1 0-.708.708L8.293 5l-7.147 7.146A.5.5 0 0 0 1 12.5v1.793l-.854.853a.5.5 0 1 0 .708.707L1.707 15H3.5a.5.5 0 0 0 .354-.146L11 7.707l1.146 1.147a.5.5 0 0 0 .708-.708l-.647-.646 3.147-3.146a1.207 1.207 0 0 0 0-1.708zM10.293 7 3.293 14H2v-1.293l7-7z"

function glyph(path: string): ReactNode {
  return (
    <svg viewBox="0 0 16 16" aria-hidden="true">
      <path d={path} />
    </svg>
  )
}
//#endregion

//#region Descriptors
// The canonical tool order + metadata, shared by the picker, the keymap and both
// cursor renderings.
export const TOOLS: ToolDescriptor[] = [
  {
    id: "pencil",
    name: "Pencil",
    shortcut: "P",
    usesStroke: true,
    iconPath: PENCIL_PATH,
    // Drawn tip-down-left, so the point is the bottom-left corner.
    hotspot: [1, 15],
    icon: glyph(PENCIL_PATH),
  },
  {
    id: "eraser",
    name: "Eraser",
    shortcut: "E",
    usesStroke: true,
    iconPath: ERASER_PATH,
    hotspot: [2, 14],
    icon: glyph(ERASER_PATH),
  },
  {
    id: "bucket",
    name: "Fill",
    shortcut: "F",
    usesStroke: false,
    iconPath: BUCKET_PATH,
    // The spout, where the paint actually lands.
    hotspot: [1, 9],
    icon: glyph(BUCKET_PATH),
  },
  {
    id: "spray",
    name: "Spray",
    shortcut: "S",
    usesStroke: false,
    iconPath: SPRAY_PATH,
    // The nozzle at the top right, where the mist comes out.
    hotspot: [10, 4],
    icon: glyph(SPRAY_PATH),
  },
  {
    id: "eyedropper",
    name: "Eyedropper",
    shortcut: "I",
    usesStroke: false,
    iconPath: EYEDROPPER_PATH,
    hotspot: [1, 15],
    icon: glyph(EYEDROPPER_PATH),
  },
]

export const toolById = (id: AppTool): ToolDescriptor =>
  TOOLS.find((tool) => tool.id === id) ?? TOOLS[0]
//#endregion
