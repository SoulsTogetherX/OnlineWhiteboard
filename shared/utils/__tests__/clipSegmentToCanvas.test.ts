...
describe("clipSegmentToCanvas", () => {
  it("returns a fully-inside segment untouched", () => {
    expect(clipSegmentToCanvas([10, 10], [20, 30])).toEqual([
      [10, 10],
      [20, 30],
    ])
  })
  ...
  it("handles a degenerate segment (a single point) inside", () => {
    expect(clipSegmentToCanvas([5, 5], [5, 5])).toEqual([
      [5, 5],
      [5, 5],
    ])
  })
  ...
  it("rejects a single point outside", () => {
    expect(clipSegmentToCanvas([-1, 5], [-1, 5])).toBeNull()
    expect(clipSegmentToCanvas([CANVAS_WIDTH, 5], [CANVAS_WIDTH, 5])).toBeNull()
  })
  ...
  it("clips a horizontal run that leaves the right edge", () => {
    ...
    const clipped = clipSegmentToCanvas([100, 50], [500, 50])
  ...
  it("clips to the true crossing point, NOT to a per-axis clamp", () => {
    ...
    const clipped = clipSegmentToCanvas([50, 50], [200, 60])
  ...
  it("clips a segment leaving the top edge (negative y)", () => {
    const clipped = clipSegmentToCanvas([60, 40], [60, -100])
    expect(clipped).toEqual([
      [60, 40],
      [60, 0],
    ])
  ...
  it("clips a segment leaving the left edge (negative x)", () => {
    const clipped = clipSegmentToCanvas([40, 60], [-100, 60])
    expect(clipped).toEqual([
      [40, 60],
      [0, 60],
    ])
  ...
  it("clips BOTH ends when the pointer re-enters from outside", () => {
    ...
    const clipped = clipSegmentToCanvas([-50, 60], [200, 60])
    expect(clipped).toEqual([
      [0, 60],
      [MAX_X, 60],
    ])
  ...
  it("returns null when the segment misses the canvas entirely", () => {
    ...
    expect(clipSegmentToCanvas([200, 10], [300, 90])).toBeNull()
    // Passing above the canvas.
    expect(clipSegmentToCanvas([-10, -10], [200, -10])).toBeNull()
  })
  ...
  it("returns null for a diagonal that passes outside the corner", () => {
    ...
    expect(clipSegmentToCanvas([200, 0], [130, -80])).toBeNull()
  })

  it("keeps a segment that only grazes a corner", () => {
    const clipped = clipSegmentToCanvas([MAX_X, MAX_Y], [200, 200])
    expect(clipped).not.toBeNull()
  ...
  it("always returns in-bounds endpoints — the wire protocol requires it", () => {
    ...
    for (const [a, b] of cases) {
      const clipped = clipSegmentToCanvas(a, b)
      if (clipped === null) {
        continue
      }
  ...
  it("rejects non-finite input rather than emitting NaN coordinates", () => {
    expect(clipSegmentToCanvas([0, 0], [NaN, 5])).toBeNull()
    expect(clipSegmentToCanvas([0, 0], [Infinity, 5])).toBeNull()
    expect(clipSegmentToCanvas([-Infinity, 0], [5, 5])).toBeNull()
  })

  it("is order-independent about which end is outside", () => {
    const out = clipSegmentToCanvas([50, 50], [200, 60])
    const back = clipSegmentToCanvas([200, 60], [50, 50])
